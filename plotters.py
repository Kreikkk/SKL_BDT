...
def significance_plot(SDataframe, BDataframe, ratio, ndots=1000, model_id="", ROC=True):
	...
	curve.GetXaxis().SetTitle(f"Cut value applied on SKL_BDT-{model_id} output")
	...
	fig.savefig(f"pictures_sig/SKL_BDT-{model_id}_outputCut.pdf")
...
def ROC_plot(SEff, BRej, model_id=""):
	...
	curve.SetLineWidth(2)
	curve.GetXaxis().SetTitle(f"SKL_BDT-{model_id} signal efficiency")
	curve.GetYaxis().SetTitle(f"SKL_BDT-{model_id} background rejection")
	curve.SetMarkerSize(0)
	...
	fig.savefig(f"pictures_roc/SKL_BDT-{model_id}_ROC_curve.pdf")


def BDT_output_hist_plot(STestDataframe, BTestDataframe,
						 STrainDataframe, BTrainDataframe, model_id=""):

	outputs = pd.concat((STestDataframe,
						 BTestDataframe,
						 STrainDataframe,
						 BTrainDataframe), ignore_index=True)["BDToutput"]

	left = np.min(outputs)
	right = np.max(outputs)

	STestHist = root.TH1F("", "", 20, left, right)
	BTestHist = root.TH1F("", "", 20, left, right)
	STrainHist = root.TH1F("", "", 20, left, right)
	BTrainHist = root.TH1F ("", "", 20, left, right)
	...
	SYLowHist = root.TH1F("", "", 20, left, right)
	SYUpHist = root.TH1F("", "", 20, left, right)
	...
	BYLowHist = root.TH1F("", "", 20, left, right)
	BYUpHist = root.TH1F("", "", 20, left, right)
	...
	ax3.set_xlabel(f"SKL_BDT-{model_id} classifier response")
	...
	fig.savefig(f"pictures/SKL_BDT-{model_id}_output.pdf")